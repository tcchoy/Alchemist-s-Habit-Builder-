    ...
    const confirmPurchase = () => {
        ...
        if (selectedItem.effect === 'unlock_category') {
            ...
            if (buyShopItem(selectedItem, targetCategory)) {
                alert(`${targetCategory} efficiency boosted!`);
                setSelectedItem(null);
            } else {
                alert("Purchase failed.");
            }
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-surface-dark p-6 rounded-2xl w-full max-w-md border border-white/10 space-y-4 shadow-2xl">
                        ...
                        {selectedItem.effect === 'category_multiplier' && (
                            <>
                                <p className="text-gray-400 text-sm">Choose a category to receive a permanent 1.5x reward multiplier.</p>
                                <div className="space-y-2">
                                    <label className="text-xs text-gray-500 uppercase font-bold">Target Category</label>
